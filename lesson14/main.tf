...
resource "yandex_compute_instance" "private_vm" {
  ...
  provisioner "local-exec" {
    command    = "sleep 5 && ansible-playbook -D -i ./inventory_private -u ***** ${path.module}/provision/playbook_private.yml"
    on_failure = continue
  }
}
...
resource "yandex_compute_instance" "public_vm" {
  ...
  provisioner "local-exec" {
    command    = "sleep 5 && ansible-playbook -D -i ./inventory_public -u ****** ${path.module}/provision/playbook_public.yml"
    on_failure = continue
  }
}
