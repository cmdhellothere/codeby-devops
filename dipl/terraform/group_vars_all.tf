resource "local_file" "group_vars" {
  content  = <<-DOC
    ---
    # Ansible group_vars containing variable values from Terraform.
    # Generated by Terraform.

    domain_name: ${var.domain_name}
    sub_domains:
      - name: www
        ip: "${yandex_compute_instance.app.network_interface.0.ip_address}"
        port: 80
      - name: gitlab
        ip: "${yandex_compute_instance.gitlab.network_interface.0.ip_address}"
        port: 80
      - name: grafana
        ip: "${yandex_compute_instance.monitoring.network_interface.0.ip_address}"
        port: 3000
      - name: prometheus
        ip: "${yandex_compute_instance.monitoring.network_interface.0.ip_address}"
        port: 9090
      - name: alertmanager
        ip: "${yandex_compute_instance.monitoring.network_interface.0.ip_address}"
        port: 9093

    test_cert: "${var.test_cert}"
    nginx_local_ip: ${yandex_compute_instance.nginx.network_interface.0.ip_address}

    environment:
      http_proxy: 'http://{{ nginx_local_ip }}:3128'
      https_proxy: 'http://{{ nginx_local_ip }}:3128'

    gitlab_root_password: '${var.gitlab_root_password}'
    gitlab_runner_token: '${var.gitlab_runner_token}'

    exporter_binary: 'prometheus_node_exporter'
    exporter_binary_dest: '/usr/bin/prometheus_node_exporter'
    exporter_service: 'prometheus-node-exporter.service'
    exporter_service_dest: '/etc/systemd/system/prometheus-node-exporter.service'
    exporter_upstart: 'prometheus-node-exporter.conf'
    exporter_upstart_dest: '/etc/init/prometheus-node-exporter.conf'

    server_binary: 'prometheus'
    server_binary_dest: '/usr/bin/prometheus'
    server_service: 'prometheus.service'
    server_service_dest: '/etc/systemd/system/prometheus.service'

    prometheus_user: 'prometheus'
    prometheus_server_name: 'prometheus'

    client_information_dict:
        '{{ domain_name }}': '${yandex_compute_instance.nginx.network_interface.0.ip_address}:9100'
        'db01.{{ domain_name }}': '${yandex_compute_instance.db01.network_interface.0.ip_address}:9100'
        'db02.{{ domain_name }}': '${yandex_compute_instance.db02.network_interface.0.ip_address}:9100'
        'app.{{ domain_name }}': '${yandex_compute_instance.app.network_interface.0.ip_address}:9100'
        'gitlab.{{ domain_name }}': '${yandex_compute_instance.gitlab.network_interface.0.ip_address}:9100'
        'runner.{{ domain_name }}': '${yandex_compute_instance.runner.network_interface.0.ip_address}:9100'
        'monitoring.{{ domain_name }}': '${yandex_compute_instance.monitoring.network_interface.0.ip_address}:9100'
    grafana_password: '${var.grafana_password}'



    DOC
  filename = "../ansible/group_vars/all.yml"

  depends_on = [
    yandex_compute_instance.nginx,
    yandex_compute_instance.db01,
    yandex_compute_instance.db02,
    yandex_compute_instance.app,
    yandex_compute_instance.gitlab,
    yandex_compute_instance.runner,
    yandex_compute_instance.monitoring
  ]
}